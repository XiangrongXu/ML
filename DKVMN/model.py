...
class LogisticRegressionMaskOutputProp(mx.operator.CustomOpProp):
    def __init__(self, ignore_label):
        ...
        self.ignore_lable = safe_eval(ignore_label)
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return LogisticRegressionMaskOutput(ignore_label=self.ignore_lable)

def logistic_regression_mask_output(data, label, ignore_label, name=None):
    return mx.sym.Custom(name=name, op_tpye="LogisticRegressionOutput", ignore_label=ignore_label, data=data, label=label)

class MODEL(object):
    def __init__(self, n_question, seqlen, batch_size, q_embed_dim, qa_embed_dim, memory_size, memory_key_state_dim, memory_value_state_dim, final_fc_dim, name="KT"):
    ...
    def sys_gen(self):
        q_data = mx.sym.Variable("q_data", shape=(self.seqlen, self.batch_size))
        ...
        q_embed_data = mx.sym.Embedding(data=q_data,
                                        input_dim=self.n_question + 1,
                                        output_dim=self.q_embed_dim,
                                        name="q_embed")
        slice_q_embed_data = mx.sym.SliceChannel(q_embed_data,
                                                num_outputs=self.seqlen,
                                                axis=0,
                                                squeeze_axis=True)

        qa_data = mx.sym.BlockGrad(qa_data)
        qa_embed_data = mx.sym.Embedding(data=qa_data,
                                        input_dim=self.n_question * 2 + 1,
                                        output_dim=self.qa_embed_dim,
                                        name="qa_embed")
        slice_qa_embed_data = mx.sym.SliceChannel(qa_embed_data,
                                                num_outputs=self.seqlen,
                                                axis=0,
                                                squeeze_axis=True)
        ...
        for i in range(self.seqlen):
            ...
            read_content = mem.read(correlation_weight)
        ...
        all_read_value_content = mx.sym.Concat(*value_read_content_1, num_args=self.seqlen, dim=0)
        
        input_embed_content = mx.sym.Concat(*input_embed_1, num_args=self.seqlen, dim=0)
        ...
        read_content_embed = mx.sym.FullyConnected(data=mx.sym.Concat(all_read_value_content,
                                                                    input_embed_content,
                                                                    num_args=2, dim=1),
                                                    num_hidden=self.final_fc_dim,
                                                    name="read_content_embed")
